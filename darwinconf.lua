...
function generate_full_c()
    ...
    darwin.load_lualib_from_c(
        "luaopen_private_lua_c_silverchain_cinterop",
        "private_lua_c_silverchain_cinterop"
    )

    local full_clib = darwin.generate_c_lib_code({
        libname = "lua_c_silverchain",
        object_export = "public_lua_c_c_silverchain",
        include_e_luacembed = false
    })
    full_clib = full_clib:gsub(SILVER_CHAIN_CODE, "#include")
    dtw.write_file("release/lua_c_silverchain_full.c", full_clib)
end
